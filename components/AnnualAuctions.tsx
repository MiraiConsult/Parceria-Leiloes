...
const AnnualAuctions: React.FC<AnnualAuctionsProps> = ({ leiloes, lancamentos, transactionsLoading, categories, availableYears, selectedYear, setSelectedYear }) => {
  ...
  const processedData = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
        monthlyData[i] = { auctions: [], totals: { receitaEvento: 0, comissaoPrevista: 0, comissao: 0, inscricao: 0, receitaTotal: 0, recebido: 0, aReceber: 0, despProducao: 0, despOperacional: 0, saldo: 0 } };
    }

    yearLeiloes.forEach(leilao => {
      const leilaoLancamentos = lancamentosByLeilao.get(leilao.id) || [];
      const calculated: CalculatedData = { receitaEvento: 0, comissaoPrevista: 0, comissao: 0, inscricao: 0, receitaTotal: 0, recebido: 0, aReceber: 0, despProducao: 0, despOperacional: 0, saldo: 0 };
      calculated.receitaEvento = leilao.receita_evento || 0;
      calculated.comissaoPrevista = leilao.comissao_prevista || 0;
    ...
    const annualTotal: CalculatedData = { receitaEvento: 0, comissaoPrevista: 0, comissao: 0, inscricao: 0, receitaTotal: 0, recebido: 0, aReceber: 0, despProducao: 0, despOperacional: 0, saldo: 0 };
    Object.values(monthlyData).forEach(month => {
  ...
  const handleExport = () => {
    ...
    const headers = ["Mês/Leilão", "Data", "Faturamento", "Comissão Prevista", "Comissão", "Inscrição", "Receita Total", "Recebido (Comissão)", "A Receber (Comissão)", "Desp. Produção", "Desp. Operacional", "Saldo do Leilão"];
    ...
    for (let i = 0; i < 12; i++) {
        ...
        if(month.auctions.length > 0) {
            dataForExport.push([
                ...
                month.totals.receitaEvento/100, month.totals.comissaoPrevista/100, month.totals.comissao/100, month.totals.inscricao/100, month.totals.receitaTotal/100,
            ...
            month.auctions.forEach(a => {
                dataForExport.push([
                    ...
                    a.calculated.receitaEvento/100, a.calculated.comissaoPrevista/100, a.calculated.comissao/100, a.calculated.inscricao/100, a.calculated.receitaTotal/100,
    ...
    dataForExport.push([
        "TOTAL ANUAL", "",
        processedData.annualTotal.receitaEvento/100, processedData.annualTotal.comissaoPrevista/100, processedData.annualTotal.comissao/100, processedData.annualTotal.inscricao/100, processedData.annualTotal.receitaTotal/100,
  ...
  const renderRow = (data: CalculatedData, label: React.ReactNode, isHeader = false, isFooter = false, isSubRow = false) => {
    ...
    return (
        <tr className={`${headerClass} ${footerClass} ${subRowClass}`}>
            ...
            <td className="px-4 py-3">{data.comissaoPrevista ? formatCurrency(data.comissaoPrevista) : '-'}</td>
